...
GyroscopeBiasEstimator::GyroscopeBiasEstimator()
    : accelerometer_lowpass_filter_(kAccelerometerLowPassCutOffFrequencyHz),
      ...
      accelerometer_static_counter_(
          new IsStaticCounter(kStaticFrameDetectionThreshold)),
      gyroscope_static_counter_(
          new IsStaticCounter(kStaticFrameDetectionThreshold)),
...
void GyroscopeBiasEstimator::ProcessAccelerometer(
  ...
  const bool is_low_pass_filter_init =
      accelerometer_lowpass_filter_.IsInitialized();
  ...
  accelerometer_static_counter_->AppendFrame(
      Length(smoothed_accelerometer_delta) <
...
Vector3 GyroscopeBiasEstimator::ComputeAngularVelocityFromLatestAccelerometer(
  ...
  const auto incremental_rotation = Rotation::RotateInto(
      Vector3(last_mean_filtered_accelerometer_value_[0],
              last_mean_filtered_accelerometer_value_[1],
              last_mean_filtered_accelerometer_value_[2]),
...
bool GyroscopeBiasEstimator::UpdateGyroscopeBias(
  ...
  gyroscope_bias_lowpass_filter_.AddSample(
      gyroscope_lowpass_filter_.GetFilteredData(), timestamp_ns);
...
bool GyroscopeBiasEstimator::IsCurrentEstimateValid() const {
  ...
  const auto current_gravity_dir =
      Normalized(last_mean_filtered_accelerometer_value_);
  const auto gyro_bias_lowpass =
      gyroscope_bias_lowpass_filter_.GetFilteredData();
  ...
  const bool hasEnoughSamples = current_accumulated_weights_gyroscope_bias_ >
                                kMinSumOfWeightsGyroBiasThreshold;
  const bool areCountersStatic =
      gyroscope_static_counter_->IsRecentlyStatic() &&
      accelerometer_static_counter_->IsRecentlyStatic();

  const bool isStatic = hasEnoughSamples && areCountersStatic &&
                        !isGyroscopeBiasCorrelatedWithSimulatedGyro;
  return isStatic;
}
